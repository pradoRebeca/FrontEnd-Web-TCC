...
const urlFinal = async () => {
  ...
  const idDeficiencia = document.querySelector('input[name="rdDeficiencia"]:checked');
...
const previewVaga = ({ titulo, localTrabalho, status, deficiencia, id }) => {
  const iconStatusVaga =
    status == 0 ? "../img/inativo.png" : "../img/ativo.png";
...
const carregarVaga = async () => {
  const container = document.querySelector(".candidatosConteiner");
  const vagas = await aplicarFiltro();
